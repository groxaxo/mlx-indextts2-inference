...
from mlx_indextts.models.sampling import schedule_decode_outputs
...
class UnifiedVoiceV25(UnifiedVoiceV2):
    ...
    def prepare_inputs(
        ...
        for row in range(batch_size):
            token_values = [
                ...
                if int(value)
                not in (self.start_text_token, self.stop_text_token)
            ]
            canonical = mx.array(
                [
            ...
            row_condition = (
                conditioning[0]
                if single_condition
                else conditioning[row]
            )
            padding = (
                target_len
                - row_condition.shape[0]
                - text_emb.shape[0]
            )
            if padding < 0:
                raise ValueError(
                    "prepared text exceeds the configured position budget"
                )
            pieces = []
        ...
        return (
            mx.stack(batch_embeddings, axis=0),
            mx.stack(attention_masks, axis=0),
        )
    ...
    def generation_attention_mask(
        ...
        *,
        ...
        """Combine left padding with the minimum required causal masking."""
        ...
        # The normal cached decode path has one query at the final key position,
        # so causality adds no additional exclusions. Returning the broadcastable
        # padding mask avoids per-token arange/comparison/cat graph construction.
        if query_len == 1:
            return padding
