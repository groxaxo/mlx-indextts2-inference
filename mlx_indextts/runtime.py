...
def validate_emotion_source(options: GenerateOptions, *, has_row_emotion_refs: bool = False) -> None:
    ...
    """
    auto = options.auto_emotion or options.emotion == "auto-qwen"
...
def _default_memory_limit(version: str) -> float:
    ...
    return 24.0 if version == "2.0" else 8.0
...
class TTSRuntime:
    ...
    def _resolve_auto_emotion(
        ...
        should_auto = options.auto_emotion or options.emotion == "auto-qwen"
        if not should_auto:
            return options.emotion, {
                "emotion_source": "manual" if options.emotion else "reference",
                "emotion_json": "",
                "dominant_emotion": "",
                "qwen_elapsed_s": "",
                "qwen_raw_text": "",
        ...
        result = get_qwen_emotion(qwen_model_path).inference(text)
    ...
    def generate(
        ...
        if options.target_duration:
            duration_tokens = estimate_mel_tokens_for_duration(options.target_duration, version=version)
            max_tokens = min(max_tokens, duration_tokens)
    ...
    def batch(
        ...
        if has_row_manual_emotions and (options.auto_emotion or options.emotion == "auto-qwen"):
            raise ValueError("CSV row emotion cannot be combined with auto_emotion/auto-qwen.")
        validate_emotion_source(options, has_row_emotion_refs=has_row_emotion_refs)
        emotion_payloads: list[dict[str, Any]] | None = None
        if options.auto_emotion or options.emotion == "auto-qwen":
            self.unload()
            from mlx_indextts.qwen_emotion import (
                DEFAULT_QWEN_EMOTION_MODEL,
                adaptive_emo_alpha,
            ...
            for row, speaker in zip(rows, speakers):
                ...
                qwen_inputs.append(f"{speaker}：{text_value}" if speaker else text_value)
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            if row_max_tokens:
                ...
                hard_max = options.max_tokens or (1500 if model_version == "2.0" else 800)
